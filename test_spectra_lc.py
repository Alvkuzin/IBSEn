...
# seds = []
# e_phs = []
# lc = np.zeros(tanim.size)

# for i in range(tanim.size):
def func_par(i):
    t = tanim[i]
    
    ##### define classes: ibs:IBS --> els:ElectronsOnIBS --> spec:SpectrumIBS
    ibs = IBS(beta=None, winds=winds, gamma_max=gamma, s_max=s_max, s_max_g=2., 
    ...
    return [x_sh, y_sh, dopls, trapezoid(inj, es, axis=1), 
            ...
            trapezoid(dNe_de_IBS[Nibs:2*Nibs-1, :], ibs.s[Nibs:2*Nibs-1], axis=0) / np.max(ibs.s),
            np.array([
                e_vals[np.argmax(dNe_de_IBS[i_s, :] *e_vals**2)]
                for i_s in range(x_sh.size)
                ]),
...
# seds = np.array(seds)
# e_phs = np.array(e_phs)
# lc = np.array(lc)

def init():
    ax[0, 0].plot(orb_x, orb_y)
    # ax.imshow(disk_ps, extent=[x_forp.min(), x_forp.max(), y_forp.min(), y_forp.max()],
    #        origin='lower', cmap='plasma', alpha=alpha)
    ax[0, 0].contourf(XX, YY, disk_ps, levels=n_levels, cmap=custom_cmap)
    ax[0, 0].scatter(0, 0, c='k')
    ax[0, 0].plot([np.min(orb_x), np.max(orb_x)], [0, 0], color='k', ls='--')
    ax[0, 0].plot([0, 3*orb.r_apoastr*cos(nu_los)], [0, 3*orb.r_apoastr*sin(nu_los)], color='g', ls='--')
...
def update(i):
    ...
    t = tanim[i]
    ax[0, 0].set_title(t/orb.T)
    x, y, z = orb.vector_sp(t=t)
    # nu_tr = orb.true_an(t=t)
    ax[0, 0].scatter(x, y, c='r')
    ax[0, 0].plot([0, 4*x], [0, 4*y], c='r', ls='--')
    ...
    if to_show=='whole':
        ...
        ax[0, 0].set_ylim(-orb.b*2, orb.b*2)
