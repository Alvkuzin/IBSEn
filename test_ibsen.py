"""
    Very stupid very dumb very primitive very basic test of the
    code. Just testing that everything can imported OK and simple calculations
    are performed without errors.
"""
import ibsen
...
def test_func(method='simple', ibs_ndims=(2, ), coolings=('stat_ibs',)):
    ...
    for ibs_ndim in ibs_ndims:
        print(f"-------------- IBS is {ibs_ndim}-dimensional ----------------")
        if ibs_ndim==2:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,        
        help="IBS dimensions: 2 or 3, optional, default 2. Set to -1 for both 2 and 3-dim IBS."
    )
    parser.add_argument(
        "--testall",
        type=bool,
    ...
    if int(args.ndim) == -1:
        ...
        ndims = (int(args.ndim),)
    test_func(method = str(args.method), ibs_ndims = ndims, coolings=coolings)
