...
def Denys_solver(edot_func, Q_func, t_evol, emin = 6e8, emax=5e14,
    ...
    if not parall:
        ...
        all_eavs = []
        for ee in range( len(spec_energies)-1 ):
            # print('Evolving {:.2e} eV'.format( injection_energies[ee] ))

            all_rates.append(rates[ee] * injection_energies[ee]**2 / des[ee]) # LESHA: literally dN[in a band] * E(band) **2 / dE[band] ????...
    ...
    return xx, vals * (xx*xx/dxx) #E   and   E^2 dN/dE
